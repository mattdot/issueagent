...
4. Agent executes the minimal connectivity task set for the current release scope.
   → Additional agent responsibilities are tracked separately for future features; defer without blocking the current release.
5. Action processes the triggering GitHub event, issues a GitHub GraphQL request using standard action environment variables, and records the outcome to verify repository connectivity.
   → If the GraphQL request fails: WARN "connectivity check must report observable failure details"
...
- ✅ Keep the initial feature scope limited to verifying GitHub GraphQL connectivity; defer advanced agent behaviors to future features.
...
4. **Common follow-ups** include:
   - Additional responsibilities to delegate to the AOT agent beyond connectivity checks
...
### Primary User Story
As a platform maintainer, I want a Marketplace-published core GitHub Action available at the repository root so that downstream workflows across many repositories can invoke a lightweight .NET agent that validates connectivity to the GitHub GraphQL API without bespoke setup.

### Acceptance Scenarios
1. **Given** a repository that consumes the core action manifest at the root, **When** the workflow executes on the default GitHub runner in response to a GitHub event, **Then** the action initializes the minimal container, boots the AOT .NET agent, executes the GraphQL connectivity request using standard environment configuration, and logs success within the defined startup budget.
...
3. **Given** the action encounters a runtime failure or permission issue while processing the GraphQL request, **When** the workflow completes, **Then** the action outputs a descriptive, actionable error message that tells maintainers what failed and the next remediation step.

### Edge Cases
- The .NET agent encounters a runtime failure after initialization; action must surface the failure details, fail the workflow step, and document remediation guidance in logs.
- The action cannot reach the GitHub GraphQL endpoint because of network issues or missing permissions; action must log the reason, emit an actionable error, and avoid partial success states.
...
- **FR-006**: The .NET agent MUST perform a GraphQL request against the endpoint defined by GitHub-provided environment variables to confirm connectivity to the hosting repository, and record the outcome within workflow logs.
- **FR-007**: The release scope MUST remain limited to connectivity verification (no autonomous or AI-style behaviors) while documenting future expansion as out of scope for this iteration.
...
- **FR-009**: The repo MUST document the limited responsibilities delegated to the core agent (GraphQL connectivity check only) so consuming workflows know which tasks remain out of scope for this release.
...
- **Minimal Agent Container**: Represents the published runtime environment containing the AOT .NET agent, encapsulating performance budgets, observability signals, and the approved GitHub client library required to perform the connectivity GraphQL request, distributed alongside Marketplace releases.

---

## Constitutional Alignment *(mandatory)*
- **Security-First GitHub Operations**: Ensure the action runs with least-privilege tokens and documents how secrets are avoided or minimized during agent execution. [NEEDS CLARIFICATION: confirm authentication strategy for the agent]
- **Minimum Viable Delivery**: Initial release delivers a functional core action manifest plus Marketplace listing and container image validated against agreed performance thresholds, with the .NET agent successfully executing the GraphQL connectivity check on supported events; success measured by the percentage of external repositories adopting the Marketplace action without bespoke setup.
- **Performance-Ready Execution**: Target cold-start latency remains within the "minimal" threshold and is validated on GitHub-hosted runners; include metrics in release notes. [NEEDS CLARIFICATION: define acceptable latency metric]
